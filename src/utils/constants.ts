...
export const BALL_COLOR_CYCLE: BallColor[] = [
  "blue",
  "red",
  "yellow",
  "green",
...
export function getBallSize(weight: number): number {
  ...
  return Math.round(
    BALL_SIZE_RANGE.min + t * (BALL_SIZE_RANGE.max - BALL_SIZE_RANGE.min),
  );
}
...
export const BALL_GRADIENT_SHADES: Record<
  BallColor,
...
export const HEADER_PILL_GRADIENT = [
  "rgba(36, 48, 68, 0.95)",
  "rgba(13, 20, 34, 0.95)",
] as const;
...
/** Presentation timings only. Gameplay damage and economy live in the domain layer. */
export const DURABILITY_CONFIG = {
  shakeIntensity: { light: 2, medium: 5, heavy: 9, severe: 14 },
  stageShakeMultiplier: { pristine: 1, hairline: 1.15, cracked: 1.4, critical: 1.85 },
  impactFeedbackDurationMs: 320,
  breakAnimationDurationMs: 650,
  screenFlashDurationMs: 220,
};

/** AsyncStorage key used to persist player progress. */
export const STORAGE_KEY_PROGRESS = "balance-keeper-progress";
export const STORAGE_KEY_INTERSTITIAL = "balance-keeper-interstitial";
